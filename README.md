...
- **main** — production-ready код, готовый к развертыванию в продакшн
- **stage** — предпродакшн, код для тестирования перед релизом
- **dev** — интеграционная ветка для новых фич
- **feature/*** — ветки для разработки новой функциональности (создаются на основе dev)

### Процесс разработки

1. Создайте feature-ветку на основе `dev`:
   ```bash
   git checkout dev
   git pull origin dev
...
2. Разработайте функциональность и закоммитьте изменения в feature-ветку

3. Создайте Pull Request из feature-ветки в `dev`

4. После ревью и мерджа в `dev`, изменения попадут в интеграционное окружение

5. Для релиза создайте Pull Request из `dev` в `stage` для тестирования в предпродакшн окружении

6. После успешного тестирования на `stage` создайте Pull Request из `stage` в `main`

### Автоматические релизы

При мердже Pull Request из `stage` в `main` автоматически создается новый релиз с тегом в формате `vX.Y.Z`:

- Версия инкрементируется автоматически (patch-версия увеличивается на 1)
- Релиз включает ссылку на Pull Request и описание изменений
- Теги версий следуют семантическому версионированию (Semantic Versioning)
